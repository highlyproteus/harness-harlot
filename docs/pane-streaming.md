...
Protocol v11 retains v8's coalesced, revision-aware pane delivery over the existing owner-only Unix socket and adds saved workspace lifecycle metadata, explicit empty-workspace terminal creation, and controls. The same coherent protocol also carries private history controls and terminal identity metadata; official identity artwork is compiled into the desktop and never crosses IPC or the network. It adds no listener, account, telemetry, or non-Rust terminal engine.
