...
const NEEDS_YOU_DOT_SIZE: f32 = 7.0;
...
/// What a tab's status slot shows. Ordered by urgency so aggregates over
/// several panes take the maximum.
#[derive(Clone, Copy, Debug, Default, Eq, Ord, PartialEq, PartialOrd)]
pub(crate) enum PaneIndicator {
    /// Idle, done, or exited: the slot stays empty.
    #[default]
    None,
    Running,
    NeedsYou,
...
impl PaneIndicator {
    /// Badge color for this state: needs-you orange, running accent, else dim.
...
/// The one mapping from pane activity to its indicator. An exited pane is
/// finished whatever its last status said.
pub(crate) const fn pane_indicator(status: PaneStatus, exited: bool) -> PaneIndicator {
    match activity_section(status, exited) {
        ...
        Some(ActivitySection::Done) | None => PaneIndicator::None,
    }
}

/// The fixed-size status slot: a spinning ring while running, an orange dot
/// when the pane needs the user, otherwise empty space.
pub(crate) fn render_pane_indicator(indicator: PaneIndicator) -> AnyElement {
    ...
    match indicator {
        ...
        PaneIndicator::NeedsYou => slot.child(
            div()
                .w(px(NEEDS_YOU_DOT_SIZE))
                .h(px(NEEDS_YOU_DOT_SIZE))
                .rounded_full()
...
#[cfg(test)]
mod tests {
    use super::{PaneIndicator, pane_indicator};
