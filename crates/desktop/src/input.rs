...
use crate::view_models::{
    CommandPaletteState, Modal, RenameTarget, SidebarMode, WorkspaceCreationField,
    WorkspaceCreationKind, WorkspaceCreationStep,
};
...
impl HhApp {
    pub(crate) fn execute_command(&mut self, command: AppCommand, cx: &mut Context<Self>) {
        ...
        match command {
            ...
            AppCommand::ShowNotifications => {
                self.toggle_sidebar_mode(SidebarMode::Notifications, cx);
            }
            AppCommand::ShowBots => self.toggle_sidebar_mode(SidebarMode::Bots, cx),
            ...
            "tab" if step == Some(WorkspaceCreationStep::Details) => {
                if let Some(dialog) = self.editor.modal.workspace_creation_mut() {
                    dialog.field = match (dialog.kind, dialog.field) {
                        ...
                        (WorkspaceCreationKind::Bot, WorkspaceCreationField::WorkingDir) => {
    ...
    pub(crate) fn handle_key(
        ...
        // Esc leaves Notifications or Bots like it always left Notifications,
        // except while a bot terminal has focus: agent CLIs need Esc.
        if event.keystroke.key == "escape"
            && self.sidebar.sidebar_mode != SidebarMode::Workstations
            && !self
                .layout
                .focused_pane
...
#[cfg(test)]
mod tests {
    use super::{browser_key_text, browser_url_editor_is_active};
    use uuid::Uuid;
